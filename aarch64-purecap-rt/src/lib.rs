...
/// Invalidate the Default Data Capability. Called by the code generated by the
/// `entry` macro after the grants are derived. All later memory accesses have to
/// go through PCC, CSP or a capability that was derived while DDC was valid.
#[doc(hidden)]
pub fn __null_ddc() {
    #[cfg(all(target_arch = "aarch64", target_abi = "purecap"))]
    // SAFETY: only clears authority, never creates it
    unsafe {
        core::arch::asm!("msr DDC, czr", "isb", options(nomem, nostack, preserves_flags))
    };
}
